...
RDBMS is a relational database management system which is the basis for SQL and other database systems while SQL is the language which is used to work with the database.  
1. Why do tables need a `primary key`?
Primary keys are necessary because they identify each individual data resource within your database. you can't differentiate the data without a way to make them unique. 
1. What is the name given to a table column that references the primary key on another table.
FOREIGN KEY
1. What do we need in order to have a _many to many_ relationship between two tables.
To have a many to many relationship between two tables we need to make a table with foreign keys so that the data can interact with other tables. 
